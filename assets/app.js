...
const observer=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting){document.querySelectorAll(".toc a").forEach(a=>a.classList.remove("active"));const link=document.querySelector(`.toc a[href="#${e.target.id}"]`);link?.classList.add("active");link?.scrollIntoView({block:"nearest"})}}),{rootMargin:"-30% 0px -65% 0px"});
